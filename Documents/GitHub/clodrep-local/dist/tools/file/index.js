    ...
    async execute(params, context) {
        ...
        try {
            ...
            if (context.config.security.confirmBeforeWrite) {
                const confirmed = await context.security.requestConfirmation(`Write to file: ${absolutePath}?`);
                if (!confirmed) {
                    return {
                        success: false,
...
export class GlobTool {
    ...
    async execute(params, context) {
        ...
        try {
            ...
            const allowedMatches = matches.filter(match => context.security.isPathAllowed(match, 'read'));
            return {
                success: true,
...
export class GrepTool {
    ...
    async execute(params, context) {
        ...
        try {
            ...
            const allowedFiles = files.filter(file => context.security.isPathAllowed(file, 'read'));
