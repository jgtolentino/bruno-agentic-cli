...
export class SecurityGateway {
    ...
    async validateToolCall(call, context) {
        this.logAuditEvent({
    ...
    async validateBashCommand(command) {
        ...
        for (const requiresConfirm of this.config.tools.execution.requireConfirmation) {
            if (command.includes(requiresConfirm)) {
                const confirmed = await this.requestConfirmation(`Execute command containing '${requiresConfirm}': ${command}?`);
                if (!confirmed) {
                    this.logAuditEvent({
    ...
    validateWebRequest(url) {
        try {
            ...
            if (this.config.tools.network.allowedDomains.length > 0) {
                const isAllowed = this.config.tools.network.allowedDomains.some(allowed => domain.includes(allowed));
                if (!isAllowed) {
                    this.logAuditEvent({
                        type: 'web_request_blocked',
                        url,
