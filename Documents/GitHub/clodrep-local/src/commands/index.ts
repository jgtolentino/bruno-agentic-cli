...
export default class ClodreptCommand extends Command {
  static override description = 'Claude-Parity Local CLI with MCP Bridge Integration';
  ...
  async run(): Promise<void> {
    ...
    try {
      ...
      if (flags.mode) config.execution.mode = flags.mode as any;
      if (flags.model) config.model.local.name = flags.model;
      if (flags.offline) config.execution.offline = true;
      if (flags.debug) config.logging.level = 'debug';
  ...
  private showBanner(config: any, flags: any): void {
