#!/bin/bash
# Quick script to run the Superset dashboard in Docker on an alternative port
...
PORT=8089  # Using port 8089 instead of 8088

echo -e "${BLUE}${BOLD}📊 Running Superset Dashboard${NC}"
echo -e "${YELLOW}This script will start the Superset dashboard in Docker on port $PORT${NC}"
echo
...
CONTAINER_ID=$(docker run -d -p $PORT:8088 "$DASHBOARD_IMAGE")

if [ $? -eq 0 ]; then
    echo -e "${GREEN}${BOLD}✅ Dashboard started successfully!${NC}"
    echo -e "${BLUE}Container ID: $CONTAINER_ID${NC}"
    echo
    echo -e "${BOLD}Access your dashboard at:${NC} http://localhost:$PORT"
