#!/bin/bash
# Script to deploy JuicyChat integration across all dashboards
...
SNIPPET_HTML="$DASHBOARDS_DIR/juicy-chat-snippet.html"
...
add_juicychat() {
    ...
    # Read JuicyChat snippet
    local snippet=$(cat "$SNIPPET_HTML")
    
    # Update dashboard context in snippet based on dashboard name
    snippet=$(echo "$snippet" | sed "s/type: \"retail_advisor\"/type: \"$dashboard_type\"/g")
    
    # Check if </body> tag exists
    if grep -q "</body>" "$dashboard_file"; then
        # Insert JuicyChat snippet before </body> tag
        sed "s|</body>|$snippet\n</body>|g" "$dashboard_file" > "$output_file"
    else
        # Append JuicyChat snippet to the end of the file
        cat "$dashboard_file" > "$output_file"
...
$PYTHON_CMD -m uvicorn api.juicer_query_api:app --host 0.0.0.0 --port $PORT
EOF
