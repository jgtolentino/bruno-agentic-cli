#!/bin/bash
# deploy_v2.5.0_to_azure.sh
# Script to deploy Client360 Dashboard v2.5.0 with Sprint 07 & 08 to Azure Static Web Apps
...
LOG_DIR="output"
LOG_FILE="$LOG_DIR/deployment_v2.5.0_$TIMESTAMP.log"
CHECKSUM_FILE="$LOG_DIR/checksums_v2.5.0_$TIMESTAMP.md5"
SWA_CONFIG_PATH="$DEPLOY_DIR/staticwebapp.config.json"
AZURE_RESOURCE_GROUP="rg-client360"
...
log "${BLUE}=======================================================${NC}"
log "${BLUE}= Client360 Dashboard v2.5.0 Azure Deployment         =${NC}"
log "${BLUE}=======================================================${NC}"
log "Started at: $(date)"
...
if ! ./verify_v2.5.0_deployment.sh 2>/dev/null; then
