#!/bin/bash
# Fix the /360 path deployment for Client360 Dashboard
...
echo -e "${GREEN}Fixing Client360 Dashboard deployment for the /360 path...${NC}"
...
DEPLOYMENT_LOG="logs/fix_360_deployment_${TIMESTAMP}.log"
TEMP_DIR="temp_fix_360_${TIMESTAMP}"

# Create logs directory if it doesn't exist
mkdir -p logs
mkdir -p "$TEMP_DIR"
mkdir -p "$TEMP_DIR/360"

echo -e "${YELLOW}Creating separate directories for root and /360 path...${NC}" | tee -a "$DEPLOYMENT_LOG"

# Copy deployment files to temp directory for the /360 path
cp -r "$SOURCE_DIR"/* "$TEMP_DIR/360/"

# Fix resource paths in HTML files to use absolute paths
echo -e "${YELLOW}Fixing resource paths in HTML and JS files...${NC}" | tee -a "$DEPLOYMENT_LOG"

# Update resource references in index.html
if [ -f "$TEMP_DIR/360/index.html" ]; then
  sed -i '' 's|href="css/|href="/css/|g' "$TEMP_DIR/360/index.html"
  sed -i '' 's|src="js/|src="/js/|g' "$TEMP_DIR/360/index.html"
  echo -e "${GREEN}✅ Fixed resource paths in index.html${NC}" | tee -a "$DEPLOYMENT_LOG"
fi

# Copy the main assets to the root directory
cp -r "$TEMP_DIR/360/css" "$TEMP_DIR/"
cp -r "$TEMP_DIR/360/js" "$TEMP_DIR/"
cp -r "$TEMP_DIR/360/data" "$TEMP_DIR/"
mkdir -p "$TEMP_DIR/assets"
cp -r "$SOURCE_DIR/screenshots" "$TEMP_DIR/assets/" 2>/dev/null || echo "No screenshots directory"

# Create a proper landing page for the root
echo -e "${YELLOW}Creating landing page for the root...${NC}" | tee -a "$DEPLOYMENT_LOG"
...
# Create proper Azure Static Web App configuration
echo -e "${YELLOW}Creating updated Azure Static Web App configuration...${NC}" | tee -a "$DEPLOYMENT_LOG"
...
cat > "$TEMP_DIR/staticwebapp.config.json" << 'EOF'
{
  ...
  "navigationFallback": {
    "rewrite": "/index.html",
    "exclude": ["/css/*", "/js/*", "/data/*", "/assets/*", "/360/*"]
  },
  "globalHeaders": {
    "content-security-policy": "default-src 'self' 'unsafe-inline' 'unsafe-eval' https://cdn.jsdelivr.net https://*.azurewebsites.net https://unpkg.com https://*.tile.openstreetmap.org https://fonts.googleapis.com https://fonts.gstatic.com;",
...
echo -e "${YELLOW}📦 Creating fixed deployment package...${NC}" | tee -a "$DEPLOYMENT_LOG"
DEPLOY_ZIP="output/client360_fixed_360_deploy_${TIMESTAMP}.zip"
mkdir -p output
...
# Deploy to Azure
echo -e "${YELLOW}🚀 Deploying fixed configuration to Azure Static Web App: $APP_NAME...${NC}" | tee -a "$DEPLOYMENT_LOG"
...
if [ $DEPLOY_STATUS -eq 0 ]; then
    echo -e "${GREEN}✅ Fixed deployment completed successfully!${NC}" | tee -a "$DEPLOYMENT_LOG"
    ...
    echo -e "${GREEN}🌐 Landing page is available at: https://$DEPLOYMENT_URL/${NC}" | tee -a "$DEPLOYMENT_LOG"
    
    # Create a deployment record
    DEPLOYMENT_RECORD="reports/azure_360_fixed_deployment_${TIMESTAMP}.md"
    mkdir -p reports
    cat > "$DEPLOYMENT_RECORD" << EOL
# Azure Fixed Deployment Record - Client360 Dashboard at /360 path
...
- **Package:** $DEPLOY_ZIP
- **Log:** $DEPLOYMENT_LOG
- **Dashboard URL:** https://$DEPLOYMENT_URL/360
- **Landing Page URL:** https://$DEPLOYMENT_URL/
...
1. Landing page loads at https://$DEPLOYMENT_URL/ with proper TBWA branding
2. Dashboard loads at https://$DEPLOYMENT_URL/360 with all components
