...
class AIInsightsProvider {
  ...
  async loadParquetInsights() {
    if (!this.hasParquetSupport()) {
      console.warn('Parquet support not available, falling back to mock insights');
      return this.generateMockInsights();
    }
    
    try {
      console.log('Attempting to load parquet insights from', this.config.insightsParquetPath);
      
      // Create parquet reader instance
      const reader = new window.ParquetReader();
      
      // Read parquet file
      const insights = await reader.readParquetFile(this.config.insightsParquetPath);
      
      // Validate insights and mark as fallback if valid
      if (Array.isArray(insights) && insights.length > 0) {
        // Mark all insights as from parquet fallback
        for (const insight of insights) {
          insight.isSynthetic = true;
          insight.dataSource = 'Parquet Fallback';
        }
        console.log('Successfully loaded', insights.length, 'insights from parquet file');
        return insights;
      } else {
        console.warn('Parquet file contained no valid insights, falling back to mock insights');
        return this.generateMockInsights();
      }
    } catch (error) {
      console.error('Error loading parquet insights:', error);
      return this.generateMockInsights();
    }
  }
  ...
  /**
   * Generate mock insights when all other fallbacks fail
   * @returns {Array} - Array of mock insights
   */
  generateMockInsights() {
    console.log('Generating mock insights as last-resort fallback');
    
    // If ParquetReader is available, use its mock insights generator
    if (this.hasParquetSupport()) {
      const reader = new window.ParquetReader();
      return reader.generateMockInsights();
    }
    
    // Otherwise create our own mock insights
    return [
      {
        "category": "sales_insights",
        "InsightID": "MOCK-INSIGHT-98765",
        "entityID": "sari-001",
        "entityName": "Mock Sari-Sari Store",
        "region": "National Capital Region",
        "cityMunicipality": "Manila",
        "GeneratedAt": new Date().toISOString(),
        "title": "Sales Trend (Mock Fallback)",
        "summary": "This is a mock insight generated by the provider as a last-resort fallback.",
        "content": {
          "title": "Sales Trend (Mock Fallback)",
          "summary": "This is a mock insight generated by the provider as a last-resort fallback.",
          "details": [
            "Mock data shows 15% increase in beverages",
            "Snack sales remain consistent year-over-year",
            "Consider inventory optimization strategies"
          ],
          "recommendations": [
            "Refresh the dashboard when connectivity improves",
            "Check API configuration settings",
            "Verify data source toggle functionality"
          ],
          "confidence": 0.5,
          "priority": "medium"
        },
      ...
      },
      {
        ...
        "InsightID": "MOCK-INSIGHT-87654",
        "entityID": "brand-001",
        "entityName": "Mock Brand Analysis",
        "GeneratedAt": new Date().toISOString(),
        "title": "Brand Performance (Mock Fallback)",
        "summary": "This is a mock brand insight generated by the provider as a last-resort fallback.",
        "content": {
          "title": "Brand Performance (Mock Fallback)",
          "summary": "This is a mock brand insight generated by the provider as a last-resort fallback.",
          "sentiment_analysis": {
            "overall_score": 0.65,
            "interpretation": "This is placeholder sentiment analysis generated as a fallback."
          },
          "recommendations": [
            "Refresh the dashboard when connectivity improves",
            "Check API configuration settings",
      ...
      },
      {
        ...
        "InsightID": "MOCK-INSIGHT-76543",
        "entityID": "sari-001",
        "entityName": "Mock Store Recommendation",
        "region": "National Capital Region",
        "cityMunicipality": "Manila",
        "GeneratedAt": new Date().toISOString(),
        "title": "Store Optimization (Mock Fallback)",
        "summary": "This is a mock store recommendation generated by the provider as a last-resort fallback.",
        "content": {
          "title": "Store Optimization (Mock Fallback)",
          "summary": "This is a mock store recommendation generated by the provider as a last-resort fallback.",
          "store_assessment": {
          ...
          "action_plan": [
            {
              "action": "Check dashboard connectivity",
              "timeline": "Immediate",
              "expected_outcome": "Restore access to real insights"
            },
            {
              "action": "Verify system configuration",
              "timeline": "Within 24 hours",
              "expected_outcome": "Address any system issues"
            }
          ],
          "priority": "low"
        },
