...
echo "🚀 Deploying Client360 Dashboard to Azure from $MIRROR_BRANCH branch..."
...
if [ -f "./scripts/verify_deployment.sh" ]; then
  if ! ./scripts/verify_deployment.sh; then
    echo "❌ Deployment verification failed. Please fix issues before deploying."
    echo "📝 Check the verification log for details."
    exit 1
  else
    echo "✅ Deployment verification passed. Proceeding with deployment."
  fi
else
  echo "⚠️ Warning: Verification script not found. Continuing without verification."
  echo "ℹ️ Consider installing the verification script at ./scripts/verify_deployment.sh"
fi
    ...
    if [ -f "./scripts/deploy_tbwa_theme.sh" ]; then
      ...
      if [ -f "src/themes/tbwa.scss" ]; then
        if grep -q "rollback-dashboard" "src/themes/tbwa.scss"; then
          ...
          # Create a backup of the theme file before attempting to build
          cp "src/themes/tbwa.scss" "src/themes/tbwa.scss.bak-${TIMESTAMP}"
          
          # Ensure variables-tbwa.scss doesn't have duplicated rollback component CSS variables
          if [ -f "src/styles/variables-tbwa.scss" ]; then
            # Verify there aren't duplicate rollback variables by checking the line count
            ROLLBACK_VAR_COUNT=$(grep -c "rollback-" "src/styles/variables-tbwa.scss")
            if [ "$ROLLBACK_VAR_COUNT" -gt 20 ]; then
              echo "⚠️ Detected possible duplicate rollback variables in variables-tbwa.scss. Fixing..." | tee -a "$DEPLOYMENT_LOG"
              # Create a backup of the original file
              cp "src/styles/variables-tbwa.scss" "src/styles/variables-tbwa.scss.bak-${TIMESTAMP}"
              # Fix the file by removing duplicate lines
              awk '!seen[$0]++' "src/styles/variables-tbwa.scss" > "src/styles/variables-tbwa.scss.tmp"
              mv "src/styles/variables-tbwa.scss.tmp" "src/styles/variables-tbwa.scss"
              echo "✅ Fixed duplicate variables in variables-tbwa.scss" | tee -a "$DEPLOYMENT_LOG"
            fi
          fi
          
          # Try to compile using webpack if available
          if command -v npx &> /dev/null && [ -f "webpack.config.js" ]; then
            # First check if build-tbwa-theme.sh script exists and run it
            if [ -f "./scripts/build-tbwa-theme.sh" ]; then
              echo "📦 Running optimized TBWA theme build script..." | tee -a "$DEPLOYMENT_LOG"
              bash ./scripts/build-tbwa-theme.sh | tee -a "$DEPLOYMENT_LOG"
            else
              # Fallback to direct webpack command
            ...
            if [ -f "dist/tbwa.css" ]; then
              ...
              if grep -q "rollback-dashboard" "dist/tbwa.css"; then
                echo "✅ Verification: rollback-dashboard styles found in compiled CSS" | tee -a "$DEPLOYMENT_LOG"
              else
                echo "⚠️ Warning: rollback-dashboard styles missing from compiled CSS. Creating fallback styles..." | tee -a "$DEPLOYMENT_LOG"
                
                # Create a separate rollback styles CSS file
                cat > "$SOURCE_DIR/css/rollback-styles.css" << EOF
...
EOF
                echo "✅ Created emergency rollback styles CSS file" | tee -a "$DEPLOYMENT_LOG"
                
                # Inject a link to the rollback styles into all HTML files
                for HTML_FILE in $(find "$SOURCE_DIR" -name "*.html"); do
                  if grep -q "<head>" "$HTML_FILE"; then
                    sed -i '' '/<head>/a\
  <link rel="stylesheet" href="/css/rollback-styles.css">' "$HTML_FILE"
                    echo "✅ Added rollback styles link to $(basename "$HTML_FILE")" | tee -a "$DEPLOYMENT_LOG"
                  fi
                done
              fi
            fi
